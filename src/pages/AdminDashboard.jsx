      ...
      <div className="flex-1 p-6">
        ...
        <Section title="🔍 Student Search & Filter">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            ...
            <select className="border p-2 rounded" onChange={(e) => setBranch(e.target.value)}>
          ...
          {filteredStudents.map((s, i) => (
            <div key={i} className="flex justify-between border-b py-2">
              <span>{s.roll} | {s.name} | {s.branch} | {s.year} Year</span>
        ...
        <Section title="🏫 Class Groups">
          {["AIML-1", "AIML-2", "CSE-3", "ECE-1"].map((g, i) => (
            <div key={i} className="flex justify-between border-b py-2">
              <span>{g}</span>
              <button onClick={() => openChat(g)} className="bg-blue-600 text-white px-2 rounded">
                Open Chat
              </button>
            </div>
          ))}
        ...
        <Section title="📚 Club Groups">
          {clubs.map((c, i) => (
            <div key={i} className="flex justify-between border-b py-2">
              <span>{c}</span>
              <button onClick={() => openChat(c)} className="bg-purple-600 text-white px-2 rounded">
                Open Chat
              </button>
            </div>
          ))}
        ...
        <Section title="👨‍🎓 Alumni Panel">
          {alumni.map((a, i) => (
            <div key={i} className="flex justify-between border-b py-2">
              <span>{a}</span>
              <button onClick={() => openChat("alumni")} className="bg-green-600 text-white px-2 rounded">
                Chat
              </button>
            </div>
          ))}
